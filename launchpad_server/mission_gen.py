...
except ImportError:  # running as a loose module (not ``python -m launchpad_server``)
    from constants import Constants
    from utils import generate_random_seed
...
def _launchpad_data_dir() -> str:
    """Frozen: ``A3LaunchPad/launchpad_data`` (exe lives in ``A3LaunchPad/bin``). Dev: nested or repo-root ``launchpad_data``."""
    if getattr(sys, "frozen", False):
        exe = os.path.abspath(sys.executable)
        a3_root = os.path.dirname(os.path.dirname(exe))
        return os.path.join(a3_root, "launchpad_data")
