...
class PboFile:
    ...
    def export(self, file, progress_callback=None):
        """Export PboFile to file."""
        if isinstance(file, str):
            with open(file, 'wb') as f:
                self._export(f, progress_callback=progress_callback)
        else:
            self._export(file, progress_callback=progress_callback)

    def _export(self, file, progress_callback=None):
        hash1 = hashlib.sha1()
        ...
        for k, v in sorted(self.filedict.items()):
            if progress_callback is not None:
                try:
                    progress_callback("writing:{}".format(v.filename.decode(errors="replace")))
                except Exception:
                    pass
...
def pbo(pbo_path, include="*", exclude="", create_pbo=False,
        ...
        update_timestamps=False, progress_callback=None):
    ...
    if create_pbo:
        ...
        if not pbo_dir:
            raise OSError("PBO output path must be absolute or include a directory.")
        ...
        with PboFile() as p:
                ...
                if os.path.isfile(f):
                    if pboprefixfile and (f == '$PBOPREFIX$'):
                        ...
                        if (fnmatch.fnmatch(f.lower(), include.lower()) and not
                            ...
                            p.add(f, open(f, 'rb'))
                            _pcall("packed:{}".format(f))
                elif recursion and os.path.isdir(f):
                    _pcall("scan:{}".format(f))
            ...
            with open(tmpfile[1], 'wb') as t:
                p.export(t, progress_callback=progress_callback)
            _pcall("export:done")
