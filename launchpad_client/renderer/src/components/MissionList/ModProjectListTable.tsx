...
const descEllipsisClass = 'inline-block max-w-full overflow-hidden text-ellipsis whitespace-nowrap'
      ...
      case 'name':
        return (
          <td className={nameTitleClass} style={{ width: w }}>
            <span className={nameTextClass} title={(project.name ?? '').trim() || undefined}>
              {(project.name ?? '').trim() || '—'}
            </span>
          </td>
        )
      case 'description':
        return (
          <td
            className={tdBase}
            style={{ width: w }}
            title={(project.description ?? '').trim() || undefined}
          >
            <span className={descEllipsisClass}>{(project.description ?? '').trim() || '—'}</span>
          </td>
        )
      case 'folder':
        return (
          <td className={tdBase} style={{ width: w }}>
            {hasFolder(project) ? (
              ...
              <span className={badgeWarn}>Not set</span>
            )}
          </td>
        )
    }
        ...
        <table className="w-full table-fixed border-collapse text-[13px]">
          ...
          <tbody>
            {projects.map((project) => (
              <tr
                key={project.id}
                className={`${trBase} ${favoriteIds.has(project.id) ? trPinned : ''}`}
